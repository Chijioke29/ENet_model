    ...
    while(1):
        ...
        for jj in batch_idxs:
            ...
            if train:
                ...
                img_augmented, label_augmented = augmented['image'], augmented['mask']
            else:
                transform = get_eval_transforms()
                img_augmented, label_augmented = img, label

            inputs.append(img_augmented)
        ...
        labels = torch.tensor(labels) # .unsqueeze(1) # new code added (unsqueeze(1)) to solve shape problem
...
def decode_segmap(image):
    ...
    label_colors = np.array([Background, Ground]).astype(np.uint8)
...
def show_images(images, in_row=True, save_path=None):
    '''
